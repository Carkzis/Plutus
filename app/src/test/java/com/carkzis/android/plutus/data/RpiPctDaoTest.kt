...
class RpiPctDaoTest {
    ...
    fun insertAllAndGetRpiRates() = runBlockingTest {
        ...
        database.rpiDao().insertAll(rpiList)

        // Get items, using just the list, rather than the LiveData for simplicity.
        val retrievedRpiList = database.rpiDao().getRpiRateList()

        MatcherAssert.assertThat(retrievedRpiList.first().value, CoreMatchers.`is`("5.0"))
    ...
    fun insertAllAddsNewRpiRecord() = runBlockingTest {
        ...
        database.rpiDao().insertAll(newRpiList)

        val retrievedRpiList = database.rpiDao().getRpiRateList()

        MatcherAssert.assertThat(retrievedRpiList.first().value, CoreMatchers.`is`("5.0"))
        MatcherAssert.assertThat(retrievedRpiList.last().value, CoreMatchers.`is`("1.0"))
    ...
    fun insertAllOverwritesOnConflict() = runBlockingTest {
        ...
        MatcherAssert.assertThat(retrievedRpiList.first().value, CoreMatchers.`is`("1.0"))
        MatcherAssert.assertThat(retrievedRpiList.size, CoreMatchers.`is`(1))
    }
